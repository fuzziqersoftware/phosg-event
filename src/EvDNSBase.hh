...
class EvDNSBase {
public:
  EvDNSBase(EventBase& base, int initialize, int fail_requests_on_destroy);
  EvDNSBase(struct evdns_base* base);
  EvDNSBase(const EvDNSBase& base);
  EvDNSBase(EvDNSBase&& base);
  EvDNSBase& operator=(const EvDNSBase& base);
  ...
  struct evdns_base* get();

protected:
  struct evdns_base* base;
  bool owned;
  bool fail_requests_on_destroy;
};
